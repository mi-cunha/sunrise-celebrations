...
export type LeadFormValues = Record<"name" | "company" | "phone" | "source" | "eventType" | "desiredDate" | "guestCount" | "budgetRange" | "responsibleId" | "notes", string>;
...
export async function createLead(_: FormState, formData: FormData): Promise<FormState> {
  ...
  if (input.budgetRange) {
    ...
    if (detailsError) return { error: "Lead criado, mas não foi possível salvar a faixa de orçamento.", values: raw, version: Date.now() };
  }
