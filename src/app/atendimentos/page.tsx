  ...
  return (
    <AppShell title="Atendimentos">
      ...
      <form className="crm-filters mt-4" method="get">
        <div><label htmlFor="atendimento-assignee">Atendente</label><select id="atendimento-assignee" name="assignee" defaultValue={activeAssignee}><option value="todos">Todos</option><option value="sem_responsavel">Sem responsável</option>{(people ?? []).map((person: { id: string; display_name: string | null }) => <option key={person.id} value={person.id}>{person.display_name ?? "Usuário"}</option>)}</select></div>
