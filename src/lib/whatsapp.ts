...
export function hasWhatsAppReviewConfig() {
  return Boolean(
    process.env.WHATSAPP_REVIEW_ENABLED === "true" && process.env.WHATSAPP_REVIEW_ACCESS_TOKEN
...
function reviewConfiguration() {
  if (!hasWhatsAppReviewConfig()) throw new Error("Ambiente de análise do app desativado neste MVP.");
