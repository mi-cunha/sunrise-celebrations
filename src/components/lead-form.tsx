...
export function LeadForm({ canAssign, people, eventTypes, leadSources }: { canAssign: boolean; people: { id: string; display_name: string | null }[]; eventTypes: Option[]; leadSources: Option[] }) {
 ...
 return <form key={state.version ?? "initial"} action={action} className="grid gap-5 md:grid-cols-2">
  ...
  <div><label htmlFor="desiredDateMode">Quando seria o evento?</label><select id="desiredDateMode" name="desiredDateMode" defaultValue={values?.desiredDateMode ?? "undefined"}><option value="exact">Data exata</option><option value="month_year">Mês e ano</option><option value="weekday">Dia da semana</option><option value="undefined">Data a definir</option></select><ErrorText errors={fieldErrors.desiredDateMode}/></div>
  <div><label htmlFor="desiredDate">Data exata</label><input id="desiredDate" name="desiredDate" type="date" aria-invalid={Boolean(fieldErrors.desiredDate)} defaultValue={values?.desiredDate} className={fieldClass(fieldErrors.desiredDate)}/><ErrorText errors={fieldErrors.desiredDate}/></div>
  <div><label htmlFor="desiredDateNote">Detalhe da data</label><input id="desiredDateNote" name="desiredDateNote" maxLength={80} placeholder="Ex.: setembro de 2027 ou sábado" defaultValue={values?.desiredDateNote} className={fieldClass(fieldErrors.desiredDateNote)}/><ErrorText errors={fieldErrors.desiredDateNote}/></div>
  <div><label htmlFor="desiredStartTime">Horário de início</label><input id="desiredStartTime" name="desiredStartTime" type="time" defaultValue={values?.desiredStartTime} className={fieldClass(fieldErrors.desiredStartTime)}/><ErrorText errors={fieldErrors.desiredStartTime}/></div>
  <div><label htmlFor="desiredDurationMinutes">Duração (horas)</label><input id="desiredDurationMinutes" name="desiredDurationMinutes" type="number" min="0.5" max="24" step="0.5" placeholder="Ex.: 4" defaultValue={values?.desiredDurationMinutes} className={fieldClass(fieldErrors.desiredDurationMinutes)}/><ErrorText errors={fieldErrors.desiredDurationMinutes}/></div>
  ...
  <div><label htmlFor="budgetRange">Faixa de orçamento</label><input id="budgetRange" name="budgetRange" maxLength={120} placeholder="Ex.: R$ 15 mil a R$ 20 mil" aria-invalid={Boolean(fieldErrors.budgetRange)} defaultValue={values?.budgetRange} className={fieldClass(fieldErrors.budgetRange)}/><ErrorText errors={fieldErrors.budgetRange}/></div>
